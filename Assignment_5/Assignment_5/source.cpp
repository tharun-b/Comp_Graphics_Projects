		...
		if (Sphere_intersect(*it, ray, &Point))
		{
			...
			*/
			//if (((!i) && Point.x < -10) || i) /* special case for background */
			{
				...
				if ((new_d > 0) && ((current_d - new_d)>(1e-2 * new_d)))
				{
					if (((Point.x - BestPoint.x) < 10) && ((Point.y - BestPoint.y) < 10) && ((Point.z - BestPoint.z) < 10))
						return TRUE;
					else
						FALSE;
				}
		...
		for (it = Light_list->begin(); it != Light_list->end(); ++it)
		{
			...
			inci_ray = Normalize(inci_ray);
			{
...

				{
					...
					visibility = TRUE;// (planevis && spherevis);
				}
...
void keyboard(unsigned char key, int x, int y)
{
	...
	case 'e':   /*  s key rotates at shoulder_angle  */
		reflectionEnabled = TRUE;
		cout << "enabled reflection, Please wait !!" << endl;
		clearFramebuffer();
		drawit();
	...
	case 'E':   /*  s key rotates at shoulder_angle  */
		...
		cout << "disabled reflection, Please wait !!" << endl;
		clearFramebuffer();
		drawit();
